...
- **`opsx:explore`**: downstream. If the confirmed intent is "I want X but I don't know how to scope it," hand off to `opsx:explore` to explore variations against the now-explicit intent.
...
- [ ] Any handoff to a downstream skill (`opsx:explore`, `spec-driven-development`) was framed in terms of the confirmed intent, not the original underspecified ask
