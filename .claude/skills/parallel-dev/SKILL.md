...
- `scripts/check_partition.py` — validates partitions and audits each thread's actual changes.
- `scripts/plan_tool.py` — manages run state, worktrees, prompts, recovery, and lifecycle bookkeeping.
- `scripts/run_codex.sh` — launches codex delegates in read-only or worktree-writing mode.
- `references/agent-prompt-template.md` — supplies the non-negotiable structure for every agent prompt.

Load `references/scripts.md` for every subcommand and flag before running any of them.
