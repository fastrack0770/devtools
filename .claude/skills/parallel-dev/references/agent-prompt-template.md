...
Threads carried by codex (`runner: "codex"`) use the same template with two adjustments: drop the **Before you start** section (codex has no Skill tool), and reword the Agent-tool line under Restrictions as "do not spawn sub-sessions and do not parallelize — execute this thread sequentially yourself". Worktree confinement, file boundary, contracts, verification, and the response format apply to codex exactly as to a subagent. Such a prompt cannot come out of `plan_tool.py build` — the tool splices one shared `tail` into every thread — so fill it in from this template by hand and write it into the thread's `prompt` field, which `build` then leaves untouched.

> Keep in sync: `scripts/plan_tool.py` holds its own verbatim copies of the **File boundary** and **Contracts** blocks below (`BOUNDARY_HEADER` / `BOUNDARY_FOOTER` / `CONTRACTS_HEADER`), because `build` splices them in itself. Edit the wording here and you must edit it there too.
